...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        /// <summary>
        ///     SELECT Job AS Praca, COUNT(1) AS LiczbaPracownikow FROM Emps GROUP BY Job;
        /// </summary>
        public static IEnumerable<object> Task7()
        {
            IEnumerable<object> result = Emps.GroupBy(e => e.Job).Select(g => new {Praca = g.Key, LiczbaPracownikow = g.Count()});
            return result;
        }
        ...
        {
            Emp result = Emps.Where(e => e.Job == "Frontend programmer").OrderByDescending(e => e.HireDate)
                .FirstOrDefault();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(new[]
            {
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (emp, dept) => new{dept.Dname,emp.Empno}).GroupBy(d => d.Dname)
                .Where(g => g.Count() > 1)
        ...
        {
            int result = arr.ToList().GroupBy(e => e).Where(g => g.Count() % 2 != 0).Select(g => g.Key).FirstOrDefault();
        ...
        {
            IEnumerable<Dept> result = Depts.GroupJoin(Emps, d => d.Deptno, e => e.Deptno, (d, e) => new { Dept = d, Emp = e })
                .Where(d => d.Emp.Count() == 5 || d.Emp.Count() == 0)
                .Select(d => d.Dept)
                .OrderBy(d => d.Dname);
            return result;
        }
